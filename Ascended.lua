local Ascended={Flags={},Theme={Main=Color3.fromRGB(255,255,255),Accent=Color3.fromRGB(255,215,0),Text=Color3.fromRGB(80,80,80),Glass=0.15},Config={Enabled=false,FileName="AscendedConfig.json"}}
...
local function createMiniButton(hub,flag,labelText,onToggle,initState)
    ...
    btnStroke.Color=Color3.fromRGB(200,200,200);btnStroke.Thickness=1.5
    ...
    lbl.TextColor3=Color3.fromRGB(60,60,60);lbl.TextXAlignment=Enum.TextXAlignment.Left
    ...
    dot.BackgroundColor3=initState and Ascended.Theme.Accent or Color3.fromRGB(200,200,200)
    ...
    bindBox.BackgroundColor3=Color3.fromRGB(235,235,235)
    bindBox.Text=savedBind;bindBox.PlaceholderText="key"
    bindBox.PlaceholderColor3=Color3.fromRGB(160,160,160)
    bindBox.TextColor3=Color3.fromRGB(60,60,60);bindBox.Font=Enum.Font.GothamBold
    ...
    bindStroke.Color=Color3.fromRGB(180,180,180);bindStroke.Thickness=1
    ...
    local function setVisual(s)
        Tween(dot,{BackgroundColor3=s and Ascended.Theme.Accent or Color3.fromRGB(200,200,200)},0.15)
...
-- ── CreateMiniButton exposed on hub ──────────────────────────────────────────
...
function Ascended:CreateTab(n)local b=Instance.new("TextButton",self.Sidebar)b.Size=UDim2.new(1,0,0,32)b.BackgroundColor3=Ascended.Theme.Accent;b.BackgroundTransparency=1;b.Text="  "..n;b.TextColor3=Color3.fromRGB(180,180,180)b.Font=Enum.Font.Gotham;b.TextSize=13;b.TextXAlignment=0;Instance.new("UICorner",b).CornerRadius=UDim.new(0,6)local pg=Instance.new("ScrollingFrame",self.Container)pg.Size=UDim2.new(1,0,1,0)pg.BackgroundTransparency=1;pg.Visible=false;pg.ScrollBarThickness=3;pg.ScrollBarImageColor3=Ascended.Theme.Accent;pg.CanvasSize=UDim2.new(0,0,0,0)local layout=Instance.new("UIListLayout",pg)layout.Padding=UDim.new(0,8)layout.SortOrder=Enum.SortOrder.LayoutOrder;layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()pg.CanvasSize=UDim2.new(0,0,0,layout.AbsoluteContentSize.Y+16)end)b.MouseButton1Click:Connect(function()for _,v in pairs(self.Container:GetChildren())do if v:IsA("ScrollingFrame")then v.Visible=false end end;for _,v in pairs(self.Sidebar:GetChildren())do if v:IsA("TextButton")then Tween(v,{BackgroundTransparency=1,TextColor3=Color3.fromRGB(180,180,180)},0.2)end end;pg.Visible=true;Tween(b,{BackgroundTransparency=0.9,TextColor3=Ascended.Theme.Accent},0.2)end)if#self.Sidebar:GetChildren()==2 then pg.Visible=true;Tween(b,{BackgroundTransparency=0.9,TextColor3=Ascended.Theme.Accent},0.2)end;local f={Lib=self}local _order=0;local function nextOrder()_order+=1;return _order end
...
function f:CreateToggle(c)
    ...
    fr.LayoutOrder=nextOrder();fr.Size=UDim2.new(1,-10,0,38);fr.BackgroundColor3=Color3.fromRGB(245,245,245)
    ...
    fr.MouseButton1Click:Connect(function()
        ...
        -- sync mini button if exists
        if c.MiniButton and MiniButtons[fl] then
            MiniButtons[fl].btn.BackgroundTransparency=s and 0 or 0.1
        end
    end)
    -- attach mini button if requested
    if c.MiniButton then
        local hub=self.Lib
        task.defer(function()
            local mini=createMiniButton(hub,fl,c.MiniButton==true and c.Name or tostring(c.MiniButton),function(ms)
                s=ms;Ascended.Flags[fl]=ms
                Tween(sw,{BackgroundColor3=ms and Ascended.Theme.Accent or Color3.fromRGB(220,220,220)},0.2)
                Tween(d,{Position=ms and UDim2.new(1,-14,0.5,-6)or UDim2.new(0,2,0.5,-6)},0.2)
                c.Callback(ms);hub:Save()
            end,s)
        end)
    end
end

function f:CreateSlider(c)local fl=c.Flag or c.Name;local v=Ascended.Flags[fl]or c.CurrentValue or c.Min;Ascended.Flags[fl]=v;local fr=Instance.new("Frame",pg)fr.LayoutOrder=nextOrder()fr.Size=UDim2.new(1,-10,0,50)fr.BackgroundColor3=Color3.fromRGB(245,245,245)Instance.new("UICorner",fr).CornerRadius=UDim.new(0,6)local t=Instance.new("TextLabel",fr)t.Size=UDim2.new(1,-20,0,20)t.Position=UDim2.new(0,10,0,5)t.BackgroundTransparency=1;t.Text=c.Name.." : "..v;t.TextColor3=Ascended.Theme.Text;t.Font=4;t.TextSize=12;t.TextXAlignment=0;local hitbox=Instance.new("TextButton",fr)hitbox.Size=UDim2.new(1,-20,0,28)hitbox.Position=UDim2.new(0,10,0,22)hitbox.BackgroundTransparency=1;hitbox.Text=""hitbox.ZIndex=5;local b=Instance.new("Frame",fr)b.Size=UDim2.new(1,-30,0,5)b.Position=UDim2.new(0,15,0,35)b.BackgroundColor3=Color3.fromRGB(220,220,220)Instance.new("UICorner",b).CornerRadius=UDim.new(1,0)local fi=Instance.new("Frame",b)fi.Size=UDim2.new((v-c.Min)/(c.Max-c.Min),0,1,0)fi.BackgroundColor3=Ascended.Theme.Accent;Instance.new("UICorner",fi).CornerRadius=UDim.new(1,0)local dot=Instance.new("Frame",fi)dot.Size=UDim2.new(0,14,0,14)dot.Position=UDim2.new(1,-7,0.5,-7)dot.BackgroundColor3=Ascended.Theme.Accent;dot.ZIndex=3;Instance.new("UICorner",dot).CornerRadius=UDim.new(1,0)local ds=Instance.new("UIStroke",dot)ds.Color=Color3.fromRGB(200,160,0)ds.Thickness=1.5;ds.Transparency=0.5;if Ascended.Flags[fl]then task.spawn(function()c.Callback(v)end)end;local function up()local m=UIS:GetMouseLocation().X;local p=math.clamp((m-b.AbsolutePosition.X)/b.AbsoluteSize.X,0,1)local val=math.floor(c.Min+(c.Max-c.Min)*p)v=val;Ascended.Flags[fl]=v;t.Text=c.Name.." : "..v;fi.Size=UDim2.new(p,0,1,0)c.Callback(v)self.Lib:Save()end;local dragging=false;hitbox.InputBegan:Connect(function(i)if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dragging=true;up()end end)UIS.InputChanged:Connect(function(i)if dragging and(i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch)then up()end end)UIS.InputEnded:Connect(function(i)if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dragging=false end end)end
function f:CreateInput(c)local fl=c.Flag or c.Name;local stored=Ascended.Flags[fl]or c.Default or""Ascended.Flags[fl]=stored;local fr=Instance.new("Frame",pg)fr.LayoutOrder=nextOrder()fr.Size=UDim2.new(1,-10,0,54)fr.BackgroundColor3=Color3.fromRGB(245,245,245)Instance.new("UICorner",fr).CornerRadius=UDim.new(0,6)local lbl=Instance.new("TextLabel",fr)lbl.Size=UDim2.new(1,-10,0,18)lbl.Position=UDim2.new(0,10,0,4)lbl.BackgroundTransparency=1;lbl.Text=c.Name;lbl.TextColor3=Ascended.Theme.Text;lbl.Font=Enum.Font.Gotham;lbl.TextSize=12;lbl.TextXAlignment=Enum.TextXAlignment.Left;local box=Instance.new("TextBox",fr)box.Size=UDim2.new(1,-20,0,24)box.Position=UDim2.new(0,10,0,24)box.BackgroundColor3=Color3.fromRGB(235,235,235)box.Text=stored;box.PlaceholderText=c.Placeholder or"Enter value..."box.PlaceholderColor3=Color3.fromRGB(180,180,180)box.TextColor3=Ascended.Theme.Text;box.Font=Enum.Font.Gotham;box.TextSize=12;box.TextXAlignment=Enum.TextXAlignment.Left;box.ClearTextOnFocus=false;Instance.new("UICorner",box).CornerRadius=UDim.new(0,5)local stroke=Instance.new("UIStroke",box)stroke.Color=Color3.fromRGB(200,200,200)stroke.Thickness=1;local pad=Instance.new("UIPadding",box)pad.PaddingLeft=UDim.new(0,6)box.Focused:Connect(function()Tween(stroke,{Color=Ascended.Theme.Accent,Thickness=1.5},0.15)end)box.FocusLost:Connect(function()Tween(stroke,{Color=Color3.fromRGB(200,200,200),Thickness=1},0.15)local val=box.Text;Ascended.Flags[fl]=val;stored=val;if c.Callback then c.Callback(val)end;self.Lib:Save()end)end
function f:CreateDropdown(c)local itemH=28;local maxVisible=5;local totalH=#c.Options*itemH;local listH=math.min(totalH,maxVisible*itemH)local closedH=38;local openH=closedH+6+listH;local fr=Instance.new("Frame",pg)fr.LayoutOrder=nextOrder()fr.Size=UDim2.new(1,-10,0,closedH)fr.BackgroundColor3=Color3.fromRGB(245,245,245)fr.ClipsDescendants=true;Instance.new("UICorner",fr).CornerRadius=UDim.new(0,6)local bt=Instance.new("TextButton",fr)bt.Size=UDim2.new(1,0,0,closedH)bt.BackgroundTransparency=1;bt.Text="  "..c.Name.." : Select"bt.TextColor3=Ascended.Theme.Text;bt.Font=Enum.Font.Gotham;bt.TextSize=13;bt.TextXAlignment=Enum.TextXAlignment.Left;local scroll=Instance.new("ScrollingFrame",fr)scroll.Size=UDim2.new(1,-10,0,listH)scroll.Position=UDim2.new(0,5,0,closedH+4)scroll.BackgroundTransparency=1;scroll.ScrollBarThickness=totalH>listH and 3 or 0;scroll.ScrollBarImageColor3=Ascended.Theme.Accent;scroll.CanvasSize=UDim2.new(0,0,0,totalH)scroll.TopImage=""scroll.MidImage=""scroll.BottomImage=""local ddLayout=Instance.new("UIListLayout",scroll)ddLayout.Padding=UDim.new(0,3)for _,o in pairs(c.Options)do local opt=Instance.new("TextButton",scroll)opt.Size=UDim2.new(1,0,0,itemH-3)opt.BackgroundColor3=Color3.fromRGB(235,235,235)opt.Text="  "..o;opt.TextColor3=Ascended.Theme.Text;opt.Font=Enum.Font.Gotham;opt.TextSize=12;opt.TextXAlignment=Enum.TextXAlignment.Left;Instance.new("UICorner",opt).CornerRadius=UDim.new(0,5)opt.MouseButton1Click:Connect(function()bt.Text="  "..c.Name.." : "..o;open=false;Tween(fr,{Size=UDim2.new(1,-10,0,closedH)},0.25)c.Callback(o)end)end;local open=false;bt.MouseButton1Click:Connect(function()open=not open;Tween(fr,{Size=UDim2.new(1,-10,0,open and openH or closedH)},0.3)end)end
function f:CreateButton(n,cb)local b=Instance.new("TextButton",pg)b.LayoutOrder=nextOrder()b.Size=UDim2.new(1,-10,0,38)b.BackgroundColor3=Color3.fromRGB(245,245,245)b.Text="  "..n;b.TextColor3=Ascended.Theme.Text;b.Font=4;b.TextSize=13;b.TextXAlignment=0;Instance.new("UICorner",b).CornerRadius=UDim.new(0,6)b.MouseButton1Click:Connect(function()Tween(b,{BackgroundColor3=Ascended.Theme.Accent,TextColor3=Color3.new(1,1,1)},0.1).Completed:Wait()Tween(b,{BackgroundColor3=Color3.fromRGB(245,245,245),TextColor3=Ascended.Theme.Text},0.2)cb()end)end
function f:CreateSection(n)local s=Instance.new("TextLabel",pg)s.LayoutOrder=nextOrder()s.Size=UDim2.new(1,-10,0,20)s.BackgroundTransparency=1;s.Text=" "..n:upper()s.TextColor3=Ascended.Theme.Accent;s.Font=Enum.Font.GothamBold;s.TextSize=11;s.TextXAlignment=Enum.TextXAlignment.Left end
...
function f:CreateInfo(c)local fr=Instance.new("Frame",pg)fr.LayoutOrder=nextOrder()fr.Size=UDim2.new(1,-10,0,38)fr.BackgroundColor3=Color3.fromRGB(240,240,240)Instance.new("UICorner",fr).CornerRadius=UDim.new(0,6)local stroke=Instance.new("UIStroke",fr)stroke.Color=Ascended.Theme.Accent;stroke.Thickness=1;stroke.Transparency=0.6;local title=Instance.new("TextLabel",fr)title.Size=UDim2.new(0.55,-10,1,0)title.Position=UDim2.new(0,10,0,0)title.BackgroundTransparency=1;title.Text=c.Title or"Info"title.TextColor3=Ascended.Theme.Text;title.Font=Enum.Font.Gotham;title.TextSize=12;title.TextXAlignment=Enum.TextXAlignment.Left;local val=Instance.new("TextLabel",fr)val.Size=UDim2.new(0.45,-10,1,0)val.Position=UDim2.new(0.55,0,0,0)val.BackgroundTransparency=1;val.Text=tostring(c.Value or"—")val.TextColor3=Ascended.Theme.Accent;val.Font=Enum.Font.GothamBold;val.TextSize=12;val.TextXAlignment=Enum.TextXAlignment.Right;val.TextTruncate=Enum.TextTruncate.AtEnd;local handle={}function handle:Set(newValue)val.Text=tostring(newValue)end;function handle:SetTitle(newTitle)title.Text=tostring(newTitle)end;return handle end
return f end
return Ascended
