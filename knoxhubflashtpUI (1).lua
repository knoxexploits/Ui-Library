...
local function createMiniButton(hub,flag,labelText,onToggle,initState)
    ...
    -- update position in Flags when drag ends (written to disk next Hub:Save())
...
function Ascended.new(cfg)
	...
	self:Load() -- always load last saved config on startup
	...
	-- save config button
	local saveBtn=Instance.new("TextButton",titleBar)
	saveBtn.Size=UDim2.new(0,38,0,16);saveBtn.Position=UDim2.new(1,-102,0.5,-8)
	saveBtn.Text="SAVE";saveBtn.TextSize=9;saveBtn.Font=Enum.Font.GothamBold
	saveBtn.TextColor3=Color3.fromRGB(180,255,180);saveBtn.BackgroundColor3=Color3.fromRGB(20,40,20)
	saveBtn.AutoButtonColor=false;saveBtn.ZIndex=5
	Instance.new("UICorner",saveBtn).CornerRadius=UDim.new(0,4)
	local saveBtnStroke=Instance.new("UIStroke",saveBtn);saveBtnStroke.Color=Color3.fromRGB(60,120,60);saveBtnStroke.Thickness=1

	-- reset config button
	local resetBtn=Instance.new("TextButton",titleBar)
	resetBtn.Size=UDim2.new(0,38,0,16);resetBtn.Position=UDim2.new(1,-62,0.5,-8)
	resetBtn.Text="RESET";resetBtn.TextSize=9;resetBtn.Font=Enum.Font.GothamBold
	resetBtn.TextColor3=Color3.fromRGB(255,180,180);resetBtn.BackgroundColor3=Color3.fromRGB(40,20,20)
	resetBtn.AutoButtonColor=false;resetBtn.ZIndex=5
	Instance.new("UICorner",resetBtn).CornerRadius=UDim.new(0,4)
	local resetBtnStroke=Instance.new("UIStroke",resetBtn);resetBtnStroke.Color=Color3.fromRGB(120,60,60);resetBtnStroke.Thickness=1
	...
	pcall(function()wmGui.Parent=(gethui and gethui())or CG end)
	if not wmGui.Parent then wmGui.Parent=game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")end
...
-- ── NOTIFY ─────────────────────────────────────────────────────────────────
function Ascended:Notify(c)local nH=self.ScreenGui:FindFirstChild("NH")or Instance.new("Frame",self.ScreenGui)nH.Name="NH"nH.Size=UDim2.new(0,250,1,0)nH.Position=UDim2.new(1,-260,0,-10)nH.BackgroundTransparency=1;local l=nH:FindFirstChild("L")or Instance.new("UIListLayout",nH)l.VerticalAlignment=Enum.VerticalAlignment.Bottom;l.Padding=UDim.new(0,10)local n=Instance.new("Frame",nH)n.Size=UDim2.new(1,0,0,60)n.BackgroundColor3=Ascended.Theme.Main;Instance.new("UICorner",n).CornerRadius=UDim.new(0,8)local s=Instance.new("UIStroke",n)s.Color=Ascended.Theme.Accent;s.Thickness=1.5;s.Transparency=0.5;local t=Instance.new("TextLabel",n)t.Size=UDim2.new(1,-10,0,25)t.Position=UDim2.new(0,10,0,5)t.Text=c.Title;t.TextColor3=Color3.fromRGB(255,255,255);t.Font=Enum.Font.GothamBold;t.BackgroundTransparency=1;t.TextXAlignment=0;t.TextScaled=true;Instance.new("UITextSizeConstraint",t).MaxTextSize=16;local d=Instance.new("TextLabel",n)d.Size=UDim2.new(1,-10,0,25)d.Position=UDim2.new(0,10,0,25)d.Text=c.Content;d.TextColor3=Ascended.Theme.Text;d.Font=4;d.BackgroundTransparency=1;d.TextXAlignment=0;d.TextScaled=true;Instance.new("UITextSizeConstraint",d).MaxTextSize=14;Tween(n,{Position=UDim2.new(0,0,0,0)},0.4)task.delay(c.Duration or 5,function()Tween(n,{Position=UDim2.new(1.5,0,0,0)},0.4).Completed:Wait()n:Destroy()end)end

-- Hub:Save()        → saves current Flags (toggles, sliders, mini btn positions) to file
-- Hub:Load()        → reloads flags from file (called automatically on startup)
-- Hub:ResetConfig() → wipes the file and clears all flags in memory
function Ascended:Save()pcall(function()writefile(self.Config.FileName,HS:JSONEncode(Ascended.Flags))end)end
function Ascended:Load()pcall(function()if not isfile(self.Config.FileName)then return end;local raw=readfile(self.Config.FileName);if not raw or raw==""then return end;local d=HS:JSONDecode(raw);if type(d)=="table"then for i,v in pairs(d)do Ascended.Flags[i]=v end end end)end
function Ascended:ResetConfig()pcall(function()writefile(self.Config.FileName,"{}")end);for k in pairs(Ascended.Flags)do Ascended.Flags[k]=nil end end
		...
		if c.MiniButton then
			...
			task.defer(function()
				...
				miniHandle=createMiniButton(hub,mbFlag,
					...
					function(ms)c.Callback(ms)end,mbInit)
